...
namespace LongLockerNames.Patches
{
	...
	public static class uGUI_SignInput_Awake_Patch
	{
		...
		public static void AddColors(uGUI_SignInput __instance)
		{
			...
			if (originalColorCount > 8)
			{
				// AddColors has already been applied previously
				return;
			}
	...
	[HarmonyPatch(typeof(uGUI_SignInput), "colorIndex", MethodType.Setter)]
	class uGUI_SignInput_colorIndex_Patch
	{
		[HarmonyPrefix]
		static bool Prefix(uGUI_SignInput __instance)
		{
			uGUI_SignInput_Awake_Patch.AddColors(__instance);

			return true;
		}
	}

	[HarmonyPatch(typeof(uGUI_SignInput), "text", MethodType.Setter)]
	class uGUI_SignInput_text_Patch
	{
		[HarmonyPrefix]
		static bool Prefix(uGUI_SignInput __instance, string value)
		{
			if (__instance.inputField.characterLimit == 10)
			{
				__instance.inputField.characterLimit = Mod.config.SmallLockerTextLimit;
			}
			else if (__instance.inputField.characterLimit == 20)
			{
				__instance.inputField.characterLimit = Mod.config.SignTextLimit;
			}
